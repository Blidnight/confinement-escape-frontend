...
export abstract class AnimationLoader {
	...
	public static reset() {
		AnimationLoader.loadingAnimation = new Map();
		AnimationLoader.loadedAnimation  = new Set();
	...
	public static startLoadAnimation(animation: string) {
		...
		const [scene, emitter] = AnimationLoader.loadingAnimation.get(animation) as [Phaser.Scene, EventEmitter];
