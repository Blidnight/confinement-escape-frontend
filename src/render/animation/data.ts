...
export class AnimationData {
  ...
  public actionsFrames: Map<number, Map<number, { x: number; y: number }>> =
    new Map();
  public actionNames : Map<string, number> = new Map();

  public constructor(public scene: Phaser.Scene, public animation: string) {
    this._init();
    AnimationData.datas.set(animation, this);
  }
  ...
  private _initAction(data: Buffer) {
    ...
    const [nameOffset, actionNameBuffer] = decodeChunk(2, data);
    const actionName = actionNameBuffer.toString('utf-8');
    const [offset, metadataBuffer] = decodeChunk(nameOffset, data);
    ...
    this.actionNames.set(actionName, actionID);
